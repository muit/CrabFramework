...
namespace PicaVoxel
{
    ...
    public class Frame : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public void SetVoxelAtArrayPosition(int x, int y, int z, Voxel vox)
        {
            ...
            chunks[x / Chunk.CHUNK_SIZE, y / Chunk.CHUNK_SIZE, z / Chunk.CHUNK_SIZE].IsUpdated = true;

            // If we're at the edge of a chunk, we should update the next voxel as well
            if (x % Chunk.CHUNK_SIZE == 0 && (x - 1) >=0) chunks[(x - 1) / Chunk.CHUNK_SIZE, y / Chunk.CHUNK_SIZE, z / Chunk.CHUNK_SIZE].IsUpdated = true;
            if (x % Chunk.CHUNK_SIZE == Chunk.CHUNK_SIZE - 1 && (x + 1) < XSize) chunks[(x + 1) / Chunk.CHUNK_SIZE, y / Chunk.CHUNK_SIZE, z / Chunk.CHUNK_SIZE].IsUpdated = true;
            if (y % Chunk.CHUNK_SIZE == 0 && (y - 1) >= 0) chunks[x / Chunk.CHUNK_SIZE, (y - 1) / Chunk.CHUNK_SIZE, z / Chunk.CHUNK_SIZE].IsUpdated = true;
            if (y % Chunk.CHUNK_SIZE == Chunk.CHUNK_SIZE - 1 && (y + 1) < YSize) chunks[x / Chunk.CHUNK_SIZE, (y + 1) / Chunk.CHUNK_SIZE, z / Chunk.CHUNK_SIZE].IsUpdated = true;
            if (z % Chunk.CHUNK_SIZE == 0 && (z - 1) >= 0) chunks[x / Chunk.CHUNK_SIZE, y / Chunk.CHUNK_SIZE, (z - 1) / Chunk.CHUNK_SIZE].IsUpdated = true;
            if (z % Chunk.CHUNK_SIZE == Chunk.CHUNK_SIZE - 1 && (z + 1) < ZSize) chunks[x / Chunk.CHUNK_SIZE, y / Chunk.CHUNK_SIZE, (z + 1) / Chunk.CHUNK_SIZE].IsUpdated = true;
        ...
        public void UpdateChunks(bool immediate)
        {
            ...
            for (int x = 0; x < (int) Mathf.Ceil((float) XSize/Chunk.CHUNK_SIZE); x++)
                for (int y = 0; y < (int) Mathf.Ceil((float) YSize/Chunk.CHUNK_SIZE); y++)
                    for (int z = 0; z < (int) Mathf.Ceil((float) ZSize/Chunk.CHUNK_SIZE); z++)
                    {
                        ...
                        chunks[x, y, z].GenerateMesh(EditingVoxels == null ? Voxels : EditingVoxels.Length > 0 ? EditingVoxels : Voxels,
                            ParentVolume.VoxelSize,
                            x*Chunk.CHUNK_SIZE, y*Chunk.CHUNK_SIZE, z*Chunk.CHUNK_SIZE,
                            (XSize - (x*Chunk.CHUNK_SIZE) < Chunk.CHUNK_SIZE
                                ? XSize - (x*Chunk.CHUNK_SIZE)
                                : Chunk.CHUNK_SIZE),
                            (YSize - (y*Chunk.CHUNK_SIZE) < Chunk.CHUNK_SIZE
                                ? YSize - (y*Chunk.CHUNK_SIZE)
                                : Chunk.CHUNK_SIZE),
                            (ZSize - (z*Chunk.CHUNK_SIZE) < Chunk.CHUNK_SIZE
                                ? ZSize - (z*Chunk.CHUNK_SIZE)
                                : Chunk.CHUNK_SIZE), XSize-1, YSize-1, ZSize-1,
                        ...
                        chunks[x, y, z].GenerateMesh(EditingVoxels==null?Voxels:EditingVoxels.Length>0?EditingVoxels:Voxels,
                            ParentVolume.VoxelSize,
                            x * Chunk.CHUNK_SIZE, y * Chunk.CHUNK_SIZE, z * Chunk.CHUNK_SIZE,
                            (XSize - (x * Chunk.CHUNK_SIZE) < Chunk.CHUNK_SIZE ? XSize - (x * Chunk.CHUNK_SIZE) : Chunk.CHUNK_SIZE),
                            (YSize - (y * Chunk.CHUNK_SIZE) < Chunk.CHUNK_SIZE ? YSize - (y * Chunk.CHUNK_SIZE) : Chunk.CHUNK_SIZE),
                            (ZSize - (z * Chunk.CHUNK_SIZE) < Chunk.CHUNK_SIZE ? ZSize - (z * Chunk.CHUNK_SIZE) : Chunk.CHUNK_SIZE), XSize-1, YSize-1, ZSize-1,
                            ParentVolume.SelfShadingIntensity,ParentVolume.MeshingMode,ParentVolume.MeshColliderMeshingMode, immediate);
#endif
                    }
        }
        ...
        public void SaveChunkMeshes()
        {
#if UNITY_EDITOR
            if (string.IsNullOrEmpty(ParentVolume.AssetGuid)) ParentVolume.AssetGuid = Guid.NewGuid().ToString();
            if (string.IsNullOrEmpty(AssetGuid)) AssetGuid = Guid.NewGuid().ToString();
        ...
        public void SetChunkAtVoxelPositionDirty(int x, int y, int z)
        {
            ...
            PicaVoxelPoint chunkPos = new PicaVoxelPoint((int)(x / Chunk.CHUNK_SIZE), (int)(y / Chunk.CHUNK_SIZE),
