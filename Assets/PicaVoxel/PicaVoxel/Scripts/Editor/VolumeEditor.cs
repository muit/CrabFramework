...
namespace PicaVoxel
{
    ...
    public class VolumeEditor : Editor
    {
                         ...
                         (EditorPersistence.CursorMode == EditorCursorMode.BrushAdd && !EditorPersistence.BrushReplace)))
                    {
                                ...
                                if (propogateAllFrames)
                                {
                                    foreach (Frame f in voxelObject.Frames)
                                    {
                                        f.SetVoxelAtWorldPosition(r.GetPoint(d),
                                            new Voxel()
                                            {
                                                Active = (EditorPersistence.CursorMode != EditorCursorMode.Subtract),
                                                Color = voxelObject.PaletteColors[EditorPersistence.SelectedColor],
                                                Value = EditorPersistence.SelectedValue
                                            });
                                        f.UpdateChunks(true);
                             ...
                             EditorPersistence.CursorMode == EditorCursorMode.BrushSubtract))
                        {
                            ...
                            if (e.type == EventType.MouseMove)
                            {
                                ...
                                if (propogateAllFrames)
                                {
                                    foreach (Frame f in voxelObject.Frames)
                                    {
                                        ...
                                        Helper.CopyVoxelsInBox(ref f.Voxels, ref f.EditingVoxels,
                                            new PicaVoxelBox(previousBrushPos.X - Chunk.CHUNK_SIZE, previousBrushPos.Y - Chunk.CHUNK_SIZE, previousBrushPos.Z - Chunk.CHUNK_SIZE, previousBrushPos.X + EditorPersistence.BrushSize.X + Chunk.CHUNK_SIZE, previousBrushPos.Y + EditorPersistence.BrushSize.Y + Chunk.CHUNK_SIZE, previousBrushPos.Z + EditorPersistence.BrushSize.Z + Chunk.CHUNK_SIZE),
                                            new PicaVoxelBox(previousBrushPos.X - Chunk.CHUNK_SIZE, previousBrushPos.Y - Chunk.CHUNK_SIZE, previousBrushPos.Z - Chunk.CHUNK_SIZE, previousBrushPos.X + EditorPersistence.BrushSize.X + Chunk.CHUNK_SIZE, previousBrushPos.Y + EditorPersistence.BrushSize.Y + Chunk.CHUNK_SIZE, previousBrushPos.Z + EditorPersistence.BrushSize.Z + Chunk.CHUNK_SIZE),
                                            new PicaVoxelPoint(voxelObject.XSize, voxelObject.YSize, voxelObject.ZSize),
                                            new PicaVoxelPoint(voxelObject.XSize, voxelObject.YSize, voxelObject.ZSize),
                                            false);
                                ...
                                SetChunksInBoxDirty(new PicaVoxelBox(previousBrushPos.X - Chunk.CHUNK_SIZE,
                                    previousBrushPos.Y - Chunk.CHUNK_SIZE, previousBrushPos.Z - Chunk.CHUNK_SIZE,
                                    previousBrushPos.X + EditorPersistence.BrushSize.X + Chunk.CHUNK_SIZE,
                                    previousBrushPos.Y + EditorPersistence.BrushSize.Y + Chunk.CHUNK_SIZE,
                                    previousBrushPos.Z + EditorPersistence.BrushSize.Z + Chunk.CHUNK_SIZE));
                                                ...
                                                if (propogateAllFrames)
                                                {
                                                    foreach (Frame f in voxelObject.Frames)
                                                    {
                                                        ...
                                                        f.SetVoxelAtArrayPosition(bx, by, bz, new Voxel()
                                                        {
                                                            ...
                                                            Active =
                                                                (EditorPersistence.CursorMode !=
                                                                 EditorCursorMode.BrushSubtract)
                                                        });
        ...
        private void SetChunksInBoxDirty(PicaVoxelBox picaVoxelBox)
        {
            ...
            for(int x=picaVoxelBox.BottomLeftFront.X;x<=picaVoxelBox.TopRightBack.X;x+=Chunk.CHUNK_SIZE)
                for(int y=picaVoxelBox.BottomLeftFront.Y;y<=picaVoxelBox.TopRightBack.Y;y+=Chunk.CHUNK_SIZE)
                    for(int z=picaVoxelBox.BottomLeftFront.Z;z<=picaVoxelBox.TopRightBack.Z;z+=Chunk.CHUNK_SIZE)
                        if(propogateAllFrames)
            ...
            if (serializedObject.targetObjects.Length < 2 && !voxelObject.IsEnabledForEditing)
            {
                if (GUILayout.Button("Resize"))
                {
                    EditorResizeWindow window =
                        (EditorResizeWindow)EditorWindow.GetWindowWithRect((typeof(EditorResizeWindow)), new Rect(100, 100, 300, 180), true);
